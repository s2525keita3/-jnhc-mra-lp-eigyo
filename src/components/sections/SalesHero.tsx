...
import { salesHeroData } from '@/content/sales-hero';

const CONSULTATION_URL = "#consultation"; // 無料個別相談フォームへのリンク

export const SalesHero = () => {
  const titleLines = useMemo(() => salesHeroData.title.split('\n'), []);
          ...
          <div className="pt-6 md:pt-8">
            ...
            >
              <button className="relative px-10 py-5 bg-gradient-to-r from-baum-500 to-baum-400 text-white font-bold text-lg md:text-xl rounded-full shadow-lg shadow-baum-500/30 hover:shadow-xl hover:shadow-baum-500/40 transition-all duration-300 hover:scale-105">
                <span className="relative z-10">{salesHeroData.cta.main}</span>
              ...
              <span className="text-text-tertiary text-sm mt-3">{salesHeroData.cta.sub}</span>
            </a>
          </div>
        </div>
      </div>
